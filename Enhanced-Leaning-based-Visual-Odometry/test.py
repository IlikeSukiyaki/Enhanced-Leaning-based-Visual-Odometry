...
if __name__ == '__main__':    
	...
	for test_video in videos_to_test:
		...
		# for t in range(min(len(gt_pose), len(answer))):
		# 	try:
		# 		print("---------", t)
		# 		angle_loss = np.sum((answer[t][:3] - gt_pose[t,:3]) ** 2)
		# 		translation_loss = np.sum((answer[t][3:] - gt_pose[t,3:6]) ** 2)
		# 		loss += (100 * angle_loss + translation_loss)  # Should this be += instead of =?
		# 	except IndexError as e:
		# 		print(f"Index error at t={t}: {e}")
		# 		print(f"Length of answer: {len(answer)}")
		# 		break  # Remove or comment out this line after debugging is complete
